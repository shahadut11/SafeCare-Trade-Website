...
import { STATS, SERVICES, PRODUCTS, PRODUCT_CATEGORIES, PROJECTS, CLIENTS, TESTIMONIALS, FAQS, SITE, BRANDS, CONSULTANCY_SERVICES } from '@/lib/data'
...
export function Consultancy() {
  ...
  return (
    <section
      ...
      style={{
        background:
          'radial-gradient(ellipse 60% 60% at 20% 40%,rgba(30,80,200,0.08) 0%,transparent 60%),radial-gradient(ellipse 40% 40% at 80% 20%,rgba(255,69,0,0.06) 0%,transparent 60%),#0d0d14',
      }}
    >
      <div className="max-w-[1280px] mx-auto px-7">
        <div className="text-center max-w-lg mx-auto mb-14 reveal">
          <span className="section-label mb-4">Our Consultancy</span>

          <h2
            className="font-display font-black text-white text-4xl md:text-5xl lg:text-6xl whitespace-nowrap"
            style={{
              letterSpacing: '-0.03em',
              lineHeight: 1.1,
            }}
          >
            Engineering & <span className="grad-fire">Consultancy</span>
          </h2>

          <p className="text-white/50 mt-4">
            Professional consultancy services for fire safety, electrical
            systems, engineering assessments and regulatory approvals.
          </p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {CONSULTANCY_SERVICES.map(({ icon, title, desc }, i) => (
            <div
              key={title}
              className={`svc-card shine reveal d${i % 3}`}
              style={{
            ...
            >
              <div
                className="text-[24px] mb-5 w-[54px] h-[54px] rounded-[14px] flex items-center justify-center transition-all duration-300"
                style={{
              ...
              <h3 className="font-display font-bold text-white mb-2.5 text-lg">
                {title}
              </h3>

              <p className="font-body text-[14px] text-white/35 leading-relaxed mb-5">
                {desc}
              </p>

              <a
                href="#contact"
                className="font-body text-[13px] font-semibold flex items-center gap-1.5 no-underline transition-all duration-200 hover:gap-3"
                style={{ color: '#ff4500' }}
              >
                Get Consultancy →
              </a>
